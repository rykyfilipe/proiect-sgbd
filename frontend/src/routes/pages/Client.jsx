...
export default function Client() {
	...
	const handleCreate = (e) => {
		...
		const key1 = `${from}-${to}`;
		const key2 = `${to}-${from}`;
		const dist = distances[key1] || distances[key2] || 0;

		const exist = orders.length > 0;
	...
	return (
		<div className='w-screen h-screen px-8 py-12 flex flex-col items-center bg-[#f4f4f4] text-[#1a1a1a] font-sans overflow-x-hidden'>
			...
			<button
				className='absolute top-[55px] right-[50px] bg-black text-white px-7 py-2 rounded-3xl hover:bg-white hover:text-black hover:border cursor-pointer transition-all transition-normal'
